...
echo "Installing uv..."
(set -o pipefail; curl -fsSL https://astral.sh/uv/install.sh | sh)
...
if command -v uv &> /dev/null; then
    ...
    uv tool install pyright@latest       # Preserve the installed CLI alongside basedpyright
    ...
    uv tool install trafilatura@latest   # Extract article text
    uv tool install twitter-cli@latest   # Read X/Twitter timeline + search via browser cookies
else
    echo "uv not found after installation. Check the uv installation above." >&2
    exit 1
fi
