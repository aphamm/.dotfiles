...
For each candidate under validation, send to **both monks**. Message each monk by its agent id via the `hub` tool — a finished monk stays idle and revives with its full conviction context. If the monk is no longer available (or its persona has faded), fall back to a fresh subagent using the summary variant in the templates below. The monk validation prompt is candidate-type-specific.
