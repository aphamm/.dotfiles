...
Spawn each monk as a separate subagent — **typically Monk A and B, sometimes also C or D** if Phase 1c.1 surfaced additional poles — so each gets a clean context with full belief commitment.

Use one `task` batch (scales to N monks): one `tasks[]` item per monk, `agent` omitted (default `task` worker), shared background in the batch's required `context` field. Reference the large context-briefing document as a `local://` file path inside each monk's task text rather than inlining it. All monks in one batch run in parallel. Each monk's output lands at `agent://<id>`; persist each to its round file (`round_1_monk_a.md`, `round_1_monk_b.md`, ...) before Phase 4.
