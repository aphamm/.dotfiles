...
if [[ -n "$cwd" && -d "$cwd" ]]; then
    ...
    if [[ -n "$branch" ]]; then
        ...
        git_status="${file_count} uncommit · ${sync_status}"
    fi
fi
...
# Build output: Model · Context · Dir · Branch status
output="${model} · ${ctx} · ⌬ ${dir}"
